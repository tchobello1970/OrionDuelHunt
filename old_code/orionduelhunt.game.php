...
////////////
    ...
    */
    ...
    {
        $constell_points = self::getObjectListFromDb( "SELECT player_color color FROM player WHERE player_constel_win=0" );
        foreach( $constell_points as $constell_point )
        {
            ...
            // useless if less than 11 sqaures on board
            if( count( $color_tiles ) > 10 )
            {
                $bRedRoad = self::checkRoad( $this->canis_major, $this->eridanus, $color_tiles );
                self::dump('redRoad', $bRedRoad );
                if( !empty( $bRedRoad ) )
                {
                    self::DbQuery( "UPDATE player SET player_constel_win=1, player_score=player_score+1 WHERE player_color='".$constell_point['color']."'" );
                    self::notifyAllPlayers( 'constellation_road', clienttranslate( '${player_name} has linked Canis Major to Eradinus' ), [ 'player_name' => self::getActivePlayerName() ] );
                }
                else
                {
                    $bGreenRoad = self::checkRoad( $this->lepus, $this->gemini, $color_tiles );
                    self::dump('greenRoad', $bGreenRoad );
                    if( !empty($bGreenRoad) )
                    {
                        self::DbQuery( "UPDATE player SET player_constel_win=1, player_score=player_score+1 WHERE player_color='".$constell_point['color']."'" );
                        self::notifyAllPlayers( 'constellation_road', clienttranslate( '${player_name} has linked Gemini to Lepus' ), [ 'player_name' => self::getActivePlayerName() ] );
                    }
                    else
                    {
                        $bPurpleRoad = self::checkRoad( $this->taurus, $this->monoceros, $color_tiles );
                        self::dump('purpleRoad', $bPurpleRoad );
                        if( !empty($bPurpleRoad) )
                        {
                            self::DbQuery( "UPDATE player SET player_constel_win=1, player_score=player_score+1 WHERE player_color='".$constell_point['color']."'" );
                            self::notifyAllPlayers( 'constellation_road', clienttranslate( '${player_name} has linked Taurus to Monoceros' ), [ 'player_name' => self::getActivePlayerName() ] );
                        }
                    }
                }
            }
    ...
    {
        self::dump('start', $start);
        self::dump('color_tiles', $color_tiles);
        $starters = array_intersect( $start, $color_tiles );
        $finishers = array_intersect( $finish, $color_tiles );
        self::dump( 'starters', $starters );
        self::dump( 'finishers', $finishers );
    ...
    {
        $galaxy_points = self::getObjectListFromDb( "SELECT player_color color FROM player WHERE player_galaxy_win=0" );
        foreach( $galaxy_points as $galaxy_point )
        {
            ...
            while( count( $galaxies ) > 3 )
            {
                ...
                if( $galaxies_linked > 3 )
                {
                    self::notifyAllPlayers( 'constellation_road', clienttranslate( '${player_name} has linked ${galaxies_linked} Galaxies' ), [
    ...
    {
        $black_holes_points = self::getObjectListFromDb( "SELECT player_color color FROM player WHERE player_black_hole_win=0" );
        foreach( $black_holes_points as $black_hole_point )
        {
            ...
            while( count( $black_holes ) > 2 )
            {
                ...
                if( $black_holes_linked > 2 )
                {
                    self::notifyAllPlayers( 'constellation_road', clienttranslate( '${player_name} has linked ${black_holes_linked} Black Holes' ), [
                        'player_name' => self::getActivePlayerName(),
...
*/
        ...
        self::dump( 'vic_pts', $victory_points );
        ...
        else
        {
            ...
            if( $remaining_tiles == 0 )
            {
                ...
                $diff_blue = count( $blue_galaxies ) - count( $blue_black_holes ) - count( $orange_galaxies ) + count( $orange_black_holes );
                if( $diff_blue > 0 )
                {
                    $blue_player_id = self::getBluePlayerId();
                    self::DbQuery( "UPDATE player SET player_score_aux=$diff_blue WHERE player_id='$blue_player_id'" );
                ...
                else if( $diff_blue < 0 )
                {
                    ...
                    self::DbQuery( "UPDATE player SET player_score_aux=$diff_blue WHERE player_id='$orange_player_id'" );
...
//self::notifyAllPlayers('info', clienttranslate( 'nb_pieces ${bearer}' ), ['bearer' => $nb_pieces ] );
        ...
        else
        {
            if( count( $blue_pieces ) == 0 || count( $orange_pieces ) == 0 )
                throw new BgaUserException( "You must place a valid 3 Tiles Piece B " );
